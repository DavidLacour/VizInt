...
class ContinuousTransforms:
    ...
    def apply_transforms(self, img, transform_type=None, severity=None, return_params=False):
        """
        Apply a continuous transformation to the image
        ...
        transformed_img = transformed_img.to(original_device)
        if return_params:
...
class TransformationHealer(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        severity = torch.sigmoid(self.severity_head(x)).unsqueeze(1)  # Make [B, 1] instead of [B]
        rotation_angle = torch.tanh(self.rotation_head(x)).unsqueeze(1) * 30.0
        noise_std = torch.sigmoid(self.noise_head(x)).unsqueeze(1) * 0.5
        h_flip = torch.sigmoid(self.h_flip_head(x)).unsqueeze(1)
        """
    ...
    def apply_correction(self, images, predictions):
        ...
        # Get the predicted transformation types
        ...
        for i, img in enumerate(images):
            ...
            if transform_type == 'gaussian_noise':
                ...
                std = predictions['noise_std'][i].item()
                ...
                is_flipped = predictions['h_flip'][i].item() > 0.5
                if is_flipped:
                    to_pil = transforms.ToPILImage()
                    to_tensor = transforms.ToTensor()
                    pil_img = to_pil(img)
                ...
                translate_x = -predictions['translate_x'][i].item()  # Negative of predicted translation
                translate_y = -predictions['translate_y'][i].item() 
                shear_x = -predictions['shear_x'][i].item()  # Negative of predicted shear
    ...
    for _ in range(num_steps):
        ...
        for img in x:
            ...
            for t_idx, t_type in enumerate(['gaussian_noise', 'rotation', 'h_flip', 'affine']):
                t_img, t_params = ContinuousTransforms.apply_transforms(
                img, transform_type=t_type, return_params=True
                )
                t_img = t_img.to(device)  # Ensure image is on correct device
...
def find_optimal_batch_size(model, img_size, starting_batch_size=2000, device=None):
    ...
    # during actual training (95% of maximum)
    #safe_batch_size = max(1, int(working_batch_size * 0.95))
    safe_batch_size = max(1, int(working_batch_size //2))
    print(f"Using safe batch size: {safe_batch_size}") #divided by two because we need space for the ttt and healer 
...
def train_healer_model(dataset_path="tiny-imagenet-200", severity=1.0):
    ...
    # Training parameters
    num_epochs = 3  # Changed to 1 for debugging
    learning_rate = 5e-5
    warmup_steps = 500
...
def train_ttt_model(base_model, dataset_path="tiny-imagenet-200"):
    ...
    # Create continuous transforms for training
