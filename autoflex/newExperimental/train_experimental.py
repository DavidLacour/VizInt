#!/usr/bin/env python3
"""
Training script for experimental vision transformers with early stopping and checkpoint management.
"""
...
def get_data_loaders(data_root: str, batch_size: int, img_size: int = 224, num_workers: int = 4):
    ...
    # Check for ImageNet-style folder structure
...
def train_experimental_model(
    ...
    """Train an experimental vision transformer model with early stopping"""
    ...
    print(f"Checkpoint cleanup: {'Enabled' if cleanup_checkpoints else 'Disabled'}")
    ...
    best_model_path = checkpoint_path / "best_model_final.pt"
    ...
    wandb.log({
        f"{architecture}/final_best_val_acc": results['best_val_acc'],
        f"{architecture}/final_best_val_loss": results['best_val_loss'],
        f"{architecture}/final_epochs_trained": results['total_epochs'],
        f"{architecture}/early_stopped": results['early_stopped']
    })
...
def main():
    ...
    parser.add_argument('--architecture', type=str, default='fourier',
                       choices=['fourier', 'elfatt', 'mamba', 'hybrid', 'mixed'],  # Removed 'kan'
