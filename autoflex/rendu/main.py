...
def get_models_to_process(args, config):
    ...
    all_models = ['main', 'healer', 'ttt', 'ttt3fc', 'blended', 'blended3fc', 
                  'baseline', 'pretrained']
    ...
    enabled_models = []
    for model in models:
        if model in ['baseline', 'pretrained']:
            enabled_models.append(model)
        elif config.get(f'models.{model}.enabled', True):
            enabled_models.append(model)
...
def train_models(args, config, models_to_train):
    ...
    for model_name in models_to_train:
        ...
        # Train model
        try:
            if args.robust and model_name == 'main':
                ...
                trained_models[robust_model_name] = model
            else:
                model, history = trainer_service.train_model(
